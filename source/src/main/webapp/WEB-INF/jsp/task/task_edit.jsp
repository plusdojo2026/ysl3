	...
	<h1>タスク編集</h1>
		...
		<form method="POST" action="<c:url value='/Controller'/>">
			...
			<select name="task-status">
				<option value="0" ${editTask.status == 0 ? 'selected' : ''}>未着手</option>
				<option value="1" ${editTask.status == 1 ? 'selected' : ''}>進行中</option>
				<option value="2" ${editTask.status == 2 ? 'selected' : ''}>完了</option>
				<option value="3" ${editTask.status == 3 ? 'selected' : ''}>保留</option>
			</select> 
			
			優先度(必須)
			<select name="task-priority">
				<option value="0" ${editTask.priority == 0 ? 'selected' : ''}>高</option>
				<option value="1" ${editTask.priority == 1 ? 'selected' : ''}>中</option>
				<option value="2" ${editTask.priority == 2 ? 'selected' : ''}>低</option>
			</select> 
			
			担当者<input type="text" name="task-manager" value="${editTask.userName}"><br> 
			
			進捗率<input type="range" name="progress" min="0" max="100" value="${editTask.progress}" step="5" list="marks"><br>
			<datalist id="marks">
			...
			予算工数 <input type="text" name="task-estimated-works" value="${editTask.estimatedWorks}"><br>
			...
			<button type="submit" name="btn-id" value="task-update">保存</button>
		</form>
